...
as $$
  ...
  standard_my_groups as (
    select mg.group_id
    from my_groups mg
    inner join public.groups g on g.id = mg.group_id
    where g.group_kind = 'standard'
  ),
  ...
  select
    ...
    coalesce((
      select count(*)
      from group_net gn
      inner join standard_my_groups smg on smg.group_id = gn.group_id
      where gn.net_cents <> 0
    ), 0)::integer as unsettled_groups_count,
    coalesce((select count(*) from standard_my_groups), 0)::integer as active_groups_count
  from expense_contributions ec;
$$;
